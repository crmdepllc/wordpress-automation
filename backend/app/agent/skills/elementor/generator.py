...
_SYSTEM = (
    ...
    "footer) takes an `items` list — each item fills the item_slots, repeated "
    "once per item. Fill scalar slots via `content`. Sections that list "
    "'heading' (and often 'eyebrow'/'subheading') as content slots render a "
    "title above their items — always fill these for features/testimonials/"
    "pricing/stats/faq/badges/footer so each section reads as a distinct, "
    "titled block instead of a bare list; 'eyebrow' is a short overline label "
    "(e.g. 'WHY CHOOSE US'), not a repeat of the heading. A slot named 'icon' "
    "must be one of this exact list (the site's Elementor install only has "
    "these icons available — anything else silently breaks): {icons}. Format "
    "it as 'fas fa-<name>', e.g. 'fas fa-bolt'. A slot named 'background_color' "
    "is optional (a hex code) — omit it to use the site's default background. "
    "Alternate background_color across consecutive sections (e.g. default, "
    "then a light tint, then default, then a dark band) so the page reads as "
    "distinct blocks rather than one undifferentiated scroll — don't leave "
    "every section on the same background. If you set 'background_color' to "
    "something dark, also set 'heading_color' (where offered) to a light hex "
    "code such as '#ffffff' so text stays readable, and vice versa for a "
    "light background. A slot named 'accent_color' is optional — if omitted, "
    "buttons and icon badges fall back to a generic default that can clash "
    "with the rest of the page, so pick one bold accent hex up front and "
    "reuse the *exact same* value for every 'accent_color' slot on the page "
    "(it drives buttons, icon-circle backgrounds, and small accent details "
    "consistently) — never pick a different accent per section. Keep copy "
    "concise and real (no lorem ipsum)."
)
