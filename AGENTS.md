...
- **Required theme is Astra (`astra`, the free WordPress.org theme — not Astra Pro), every time.** Every site build must have Astra installed and active before generating Elementor layouts, alongside the plugin stack above.
- **This check runs automatically inside page generation, not as a separate step.** `wp_create_elementor_page` verifies (and installs/activates whatever's missing from) the Astra theme + the three plugins above before generating a layout — real slugs: `astra`, `elementor`, `royal-elementor-addons`, `elementskit-lite` (see `agent/skills/stack.py`). It checks status first and only acts on what's actually missing, so an already-configured site costs a handful of cheap status checks, not repeat installs.
- **Elementor is fail-closed; Astra/Royal Addons/ElementsKit are best-effort.** If Elementor itself can't be verified active, page creation aborts — nothing can be built without it. If the theme or either addon plugin fails to install, that failure is logged and page creation proceeds anyway (core Elementor widgets still work without them) — don't let one flaky plugin install block every page on a site.
