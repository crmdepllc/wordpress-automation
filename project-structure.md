...
- **Elementor skill ([app/agent/skills/elementor/](backend/app/agent/skills/elementor/)):** brief → validated `_elementor_data`. Claude fills a constrained `PageSpec` IR (never raw JSON); a deterministic builder compiles it from the real section templates in `examples/` (9 types: single/grid/stack layouts), regenerating ids; a validator rejects malformed structures before any write. Exposed as the gated `wp_create_elementor_page` tool, which writes via REST then auto-runs `wp elementor flush-css`. The templates are hand-authored reference scaffolds (per AGENTS.md rule #3, genuine editor exports remain the ideal), but have been **live-verified** end to end against a real WordPress + Elementor sandbox — including catching and fixing two bugs (a wrong Toggle-widget settings shape, and a fatal PHP TypeError from Sprint 6's theme-kit-merge write) that no offline/structural check could have found. See `progress-tracker.md`'s "Companion WP plugin + richer Elementor templates" entry.
...
- **Tests:** `pytest` — unit tests mock httpx (`respx`), SSH/subprocess, the LLM, and the DB, and use `MemorySaver` for the graph, so they pass without Docker. Integration tests are `@pytest.mark.integration` and self-skip when Docker/Postgres/WP is unavailable. **111 passing, 4 skipped** (one pre-existing, environment-specific `test_local_docker_executor_command` failure predates Sprint 7 unchanged — reproduces identically on `main`).
...
- **Planned components** _(per overview, not yet present)_: pgvector recall; gallery/team/image-split Elementor section types (need a real image-sourcing capability, not built); a real Playwright baseline (the render blocker is now fixed — capturing one just wasn't part of this fix pass); a persisted eval-history dashboard (explicitly deferred in Sprint 8).
