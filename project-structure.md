...
> **Current state:** Sprints 1–8 complete, plus a post-Sprint-8 fix pass. The backend has a LangGraph "ping" node (Sprint 1); a typed WordPress tool layer — REST client, pluggable WP-CLI executor, encrypted credentials, approval-gated tools (Sprint 3); a real orchestration graph (`plan → approve[interrupt] → snapshot → execute → report`) with Postgres-checkpointed state, `/api/tasks` endpoints, and a Celery worker (Sprint 4); an Elementor page-generation skill (Sprint 5); content/SEO/theming/plugin skills (Sprint 6); dependency-ordered multi-step decomposition with a pre-execution DB snapshot and halt-on-failure execution (Sprint 7); a scored eval suite wired into GitHub Actions as a blocking CI gate, plus a gated live/Playwright visual-regression workflow (Sprint 8); and a companion WP mu-plugin + a 9-type richer Elementor template library, live-verified against a real WordPress+Elementor sandbox (post-Sprint-8 fix — see `progress-tracker.md`) — **18 gated tools** the planner can compose. The frontend dashboard (Sprint 2) is wired off its mocks onto the real interrupt/resume flow. The whole stack boots via Docker Compose. Items marked _(planned)_ come from the overview but are not yet implemented.
...
├── wp-plugin/
│   ├── README.md           # Delivery notes (dev bind-mount vs. real-site file upload)
│   └── wpa-companion/
│       └── wpa-companion.php # Must-use plugin: registers protected meta with show_in_rest
├── backend/               # FastAPI + LangGraph agent server (Python)
└── frontend/              # Next.js dashboard / chat UI (TypeScript)
```

The repo is split into two independently-managed apps: a **Python backend** (`uv`-managed) and a **TypeScript frontend** (`npm`-managed). There is no shared root package manager. Docker Compose ties them together with the data and WordPress services for local dev. `wp-plugin/` is a third, minimal PHP surface — a single must-use plugin, not a Python or TypeScript package, so it isn't managed by either app's package manager.
...
| `wordpress` | `wordpress:6-php8.3-apache` | 8080 | Local sandbox WP site (the agent's target); bind-mounts `wpa-companion.php` as a must-use plugin |
| `wp-init` | `wordpress:cli-php8.3` | — | One-shot: installs WP + activates Elementor, then exits |
| `wpcli` | `wordpress:cli-php8.3` | — | Persistent WP-CLI container; `docker exec` target for the local WP-CLI transport; also gets the mu-plugin bind mount |
...
└── app/
    ...
    │   │   │   ├── schema.py    # PageSpec/SectionSpec IR (what Claude + Gemini fill)
    ...
    │   │   │   ├── generator.py # Two-pass: Claude fills structure/design slots (LLMGenerator), Gemini fills visible text (LLMCopyGenerator)
    │   │   │   ├── skill.py     # generate (Claude→Gemini) → build → validate pipeline
    │   │   │   └── examples/    # 11 section templates (hero/features/pricing/contact/footer/testimonials/stats/faq/cta_banner/about/badges) + README
    │   │   ├── content/   # Brief → PostDraft (title/body/terms) via Gemini
    │   │   ├── images/    # image_prompt → generated + uploaded WP media (Gemini, raw google-genai SDK)
    │   │   │   ├── generator.py # GeminiImageGenerator: prompt → PNG bytes
    │   │   │   └── resolver.py  # resolve_images: PageSpec + WP client → PageSpec with image_prompt resolved to image_url/image_id
    │   │   ├── stack.py   # ensure_required_stack: checks/installs Astra + Elementor/Royal Addons/ElementsKit before page generation (Elementor fail-closed, rest best-effort)
    ...
    └── evals/             # Sprint 8: scored golden dataset + CI gate
        ...
        └── scenarios/      # One file per skill — 32 scenarios total
            ├── elementor.py    # 7 (6 relocated from the old test_elementor_skill.py + 1 exercising the Claude/Gemini two-pass split)
            ├── content.py      # 4
            ├── images.py       # 3 — resolve_images against fakes (Gemini + WP media upload mocked, incl. graceful degradation on failure)
            ├── stack.py        # 4 — ensure_required_stack against a fake WpCli (already-satisfied/missing/Elementor-fails/optional-fails)
            ├── seo.py          # 4
            ├── theme.py        # 3
            ├── plugins.py      # 3
...
- **Dependencies:** all declared in `pyproject.toml` — `fastapi[standard]`, `langgraph`, `langgraph-checkpoint-postgres`, `anthropic`, `langchain-anthropic`, `google-genai`, `langchain-google-genai`, `sqlalchemy`/`asyncpg`/`psycopg`/`alembic`/`pgvector`, `redis`/`celery`, `fabric`/`paramiko`, `httpx`, `cryptography`, `pydantic-settings`.
...
- **WordPress integration ([app/wp/](backend/app/wp/)):** REST client for content (posts/pages/media/menus, incl. `create_menu_item` for menu assembly); a pluggable WP-CLI executor (Fabric/Paramiko SSH for real sites, `docker exec` for the local sandbox) for installs/activation/`elementor flush-css`/`db export`, plus boolean status checks (`plugin_is_installed`/`plugin_is_active`/`theme_is_installed`/`theme_is_active`) and theme install/activate (`install_theme`/`activate_theme`), all exit-code based like WP-CLI's own `is-installed`/`is-active` subcommands; encrypted per-site credential storage.
...
- **Elementor skill ([app/agent/skills/elementor/](backend/app/agent/skills/elementor/)):** brief → validated `_elementor_data`, via a two-pass generation split (per AGENTS.md's Gemini/Claude model-usage rule): `generator.py`'s `LLMGenerator` (Claude) fills a constrained `PageSpec` IR with structure and design slots only (section types/order/items/icon/colors/`image_prompt` — never raw JSON); `LLMCopyGenerator` (Gemini) then fills every visible text slot on that same IR, never touching `image_prompt`. A deterministic builder compiles the result from the real section templates in `examples/` (9 types: single/grid/stack layouts), regenerating ids and dropping any `image` widget left with an empty url (`builder.py`'s `_finalize_image_widgets`); a validator rejects malformed structures before any write. `skill.py` exposes `generate_page_spec` (structure + copy, no image resolution — used offline by tests/evals) and `generate_elementor_page` (adds build+validate, for callers that don't need images) separately from `build_and_validate`, so the gated `wp_create_elementor_page` tool can insert the stack check + image resolution between spec generation and building. That tool writes via REST then auto-runs `wp elementor flush-css`. The templates are hand-authored reference scaffolds (per AGENTS.md rule #3, genuine editor exports remain the ideal), but have been **live-verified** end to end against a real WordPress + Elementor sandbox — including the new `image` widget on `hero`/`about` (uploaded a real file, wrote a page exercising both, confirmed correct rendering and no PHP errors). See `progress-tracker.md`'s "Companion WP plugin + richer Elementor templates" and "Gemini image generation" entries.
- **Images skill ([app/agent/skills/images/](backend/app/agent/skills/images/)):** turns a Claude-set `image_prompt` design slot into a real, uploaded image. `generator.py`'s `GeminiImageGenerator` calls the raw `google-genai` SDK (not LangChain — needs direct access to inline image bytes) to produce PNG bytes from a prompt; `resolver.py`'s `resolve_images` uploads those bytes via `WordPressRestClient.upload_media()` (pre-existing method, Sprint 3) and replaces `image_prompt` with `image_url`/`image_id` on the section. Only ever called from inside the gated `wp_create_elementor_page` tool, post-approval — both the Gemini call and the WP upload are real work, so per AGENTS.md rule 1 neither may run during planning. A failed image (quota, API error, etc.) degrades that one section to no-image rather than failing the whole page (Issue 12, `issues-list.md`) — logged via `logging.getLogger("agent.skills.images")`, never raised past `resolve_images`.
- **Required stack pre-flight ([app/agent/skills/stack.py](backend/app/agent/skills/stack.py)):** per AGENTS.md's page-builder-plugin-stack rule, `ensure_required_stack` checks (and installs/activates whatever's missing from) the Astra theme (`astra`) and three plugins — `elementor`, `royal-elementor-addons`, `elementskit-lite` — before every page generation. Checks status first (`plugin_is_active`/`theme_is_active`, then `plugin_is_installed`/`theme_is_installed` only if inactive) and only acts on what's actually missing, so an already-configured site costs a handful of cheap status checks, not repeat installs. Elementor is fail-closed (`RequiredStackError` aborts page creation — nothing can be built without it); the theme and the other two plugins are best-effort (failure logged via `logging.getLogger("agent.skills.stack")`, page creation proceeds anyway). Runs as an internal precondition inside `wp_create_elementor_page`, same pattern as the auto CSS-flush and image resolution — not a separately-approved planner step. The per-item result is included in that tool's response under `stack_check` for visibility without a separate approval item.
- **Content / SEO / theming / plugin / menu skills ([app/agent/skills/](backend/app/agent/skills/)):** **content** → a `PostDraft` generated by Gemini and written via REST with find-or-create categories/tags + optional scheduling (`wp_publish_post`); **seo** → meta title/description + JSON-LD written as Yoast/RankMath post-meta over REST (`wp_apply_seo`); **theme** → a `ThemeSpec` applied via WP-CLI theme mods + a best-effort Elementor kit merge (`wp_apply_theme`) — the kit-color write now goes through WP-CLI's `--format=json` so Elementor receives a real PHP array, not a string; **plugins** → search + configure via WP-CLI (`wp_search_plugins` read, `wp_configure_plugin` write) with a recommend catalog; **menu** (Sprint 7) → `wp_assemble_menu` creates a nav menu and attaches pages as menu items over REST. All writes are gated. SEO/theme meta persistence is handled by the companion WP plugin (`wp-plugin/wpa-companion/`, registers `show_in_rest` for the exact meta keys these skills write). **18 tools total** now (the stack check and image resolution are not separate agent tools — both are internal steps of `wp_create_elementor_page`, same pattern as the auto CSS flush).
...
- **Eval suite ([app/evals/](backend/app/evals/), Sprint 8):** a scored golden dataset — 32 scenarios across all eight skills, each scored 0–100 by a weighted checklist (`scoring.py`) and rolled up per skill (`runner.py`). `scripts/run_evals.py` is both the CI gate and the report generator: it runs the same `runner.run_all()` that `tests/test_evals.py` asserts against, writes `eval-report.{md,json}`, and exits non-zero if any skill drops below its committed floor (`thresholds.py`). One scoring engine, two callers — no duplicated logic between the pytest dev loop and the CI script.
- **Config / secrets:** [backend/app/config.py](backend/app/config.py) reads all settings (Anthropic + Gemini API keys, models, DB/Redis URLs, `CREDENTIAL_ENCRYPTION_KEY`, Celery URLs, optional `LANGSMITH_API_KEY`, CORS) from the environment / `.env`. Credentials are Fernet-encrypted at rest; nothing is hardcoded.
- **Tests:** `pytest` — unit tests mock httpx (`respx`), SSH/subprocess, the LLM, and the DB, and use `MemorySaver` for the graph, so they pass without Docker. Integration tests are `@pytest.mark.integration` and self-skip when Docker/Postgres/WP is unavailable. **138 passing, 7 deselected** (one pre-existing, environment-specific `test_local_docker_executor_command` failure predates Sprint 7 unchanged — reproduces identically on `main`).
...
- **Companion WP plugin ([wp-plugin/wpa-companion/](wp-plugin/wpa-companion/)):** a must-use plugin registering `show_in_rest` for the 7 protected meta keys the codebase writes (Elementor + Yoast + RankMath + our JSON-LD) — closes a gap that had been open and deferred since Sprint 5. Bind-mounted into the Docker sandbox; delivered to real sites via Fabric/Paramiko file upload.
- **Planned components** _(per overview, not yet present)_: pgvector recall; gallery/team/image-split Elementor section types (the image-sourcing capability now exists — `agent/skills/images/`, live-verified — but only `hero`/`about` consume it so far; dedicated gallery/team/image-split section types remain future work); a real Playwright baseline (the render blocker is now fixed — capturing one just wasn't part of this fix pass); a persisted eval-history dashboard (explicitly deferred in Sprint 8).
...
- **Visual regression ([tests-visual/](frontend/tests-visual/), Sprint 8):** `@playwright/test` navigates to a live agent-generated Elementor page (`WP_BASE_URL`, `WP_ELEMENTOR_PAGE_SLUG`) and diffs a full-page screenshot against a committed baseline via `toHaveScreenshot()`. Runs only in `.github/workflows/eval-live.yml` (non-blocking). The render blocker (companion plugin + a theme-kit-merge fatal) is now fixed and live-verified — see `progress-tracker.md` — but no baseline image is committed yet; capturing one is a small follow-up, not done as part of that fix.
- **Planned components** _(per overview, not yet present)_: a real site selector (replacing the single default slug), live page preview panel; a real Playwright baseline (mechanism now unblocked, just not captured yet).
