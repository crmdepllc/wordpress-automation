...
└── app/
    ...
    └── evals/             # Sprint 8: scored golden dataset + CI gate
        ...
        └── scenarios/      # One file per skill — 27 scenarios total
            ├── elementor.py    # 7 (6 relocated from the old test_elementor_skill.py + 1 exercising the Claude/Gemini two-pass split)
            ├── content.py      # 4
            ├── images.py       # 2 — resolve_images against fakes (Gemini + WP media upload mocked)
            ├── seo.py          # 4
            ├── theme.py        # 3
            ├── plugins.py      # 3
...
- **Tests:** `pytest` — unit tests mock httpx (`respx`), SSH/subprocess, the LLM, and the DB, and use `MemorySaver` for the graph, so they pass without Docker. Integration tests are `@pytest.mark.integration` and self-skip when Docker/Postgres/WP is unavailable. **124 passing, 7 deselected** (one pre-existing, environment-specific `test_local_docker_executor_command` failure predates Sprint 7 unchanged — reproduces identically on `main`).
